...
def width_group(start_path, groups):
    start_path = start_path + '/epoch_wise/'
    cut = 'best'
    width_start = f'mcnn/width_wise/{cut}/'
    writer_40k_c = SummaryWriter(log_dir = width_start + groups[0])
    writer_40k_g = SummaryWriter(log_dir = width_start + groups[1])
    writer_50k_c = SummaryWriter(log_dir = width_start + groups[2])
    writer_50k_g = SummaryWriter(log_dir = width_start + groups[3])
    w4c_train = {}
    w4g_train = {}
    w5c_train = {}
    w5g_train = {}
...
groups =['100k_coloured', '100k_grayscale', '160k_coloured', '160k_grayscale']
width_group('celebA/mcnn', groups)
width_group('celebA/resnet', groups)